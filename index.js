...
app.get("/card", async (req, res) => {
  try {
    ...
    // Canvas
    const width = 600;
    const height = 800; // optimizado
    const canvas = createCanvas(width, height);
    const ctx = canvas.getContext("2d");

    // Fondo con ligero degradado para dar más estilo
    ...
    const spacing = 40; // espacio entre barras

    let yPos = 490; // posición inicial primera barra
    ...
    ctx.fillStyle = "#555";
    ctx.fillRect(barraX, yPos, barraWidth, barraHeight);
    const gradXP = ctx.createLinearGradient(barraX, yPos, barraX + barraWidth * xpPercent, yPos);
    gradXP.addColorStop(0, "#00aaff");
    gradXP.addColorStop(1, "#0055ff");
    ctx.fillStyle = gradXP;
    ctx.fillRect(barraX, yPos, barraWidth * xpPercent, barraHeight);

    // Decoración extra: pequeñas líneas diagonales en el fondo para textura
    ctx.strokeStyle = "#222";
    ctx.lineWidth = 1;
